"""Batting WAR (wOBA-based) and pitching WAR (RA9 + RE24)."""
...
RUNS_PER_WIN = 10.0
REPLACEMENT_RUNS_PER_PA = -20.0 / 600  # ~-0.0333 — typical FanGraphs replacement level
...
def compute_batting_war(
    ...
    """Batting WAR from wOBA.

        batting_runs = (wOBA - lg_wOBA) / woba_scale * PA
        replacement_runs = REPLACEMENT_RUNS_PER_PA * PA   (negative)
    ...
    if not result.result_rows:
        logger.warning("No wOBA data for {} — run 'compute woba' first", season)
        return 0
    ...
    for pid, pa, woba, woba_scale, lg_woba, wrc_plus in result.result_rows:
        ...
        rows.append([
            pid, season, pa,
            round(woba, 3), round(wrc_plus, 1),
            round(batting_runs, 1), round(replacement_runs, 1), round(war, 1),
        ])
    ...
    if rows:
        ...
        client.insert(
            ...
            column_names=[
                "player_id", "season", "pa",
                "woba", "wrc_plus",
                "batting_runs", "replacement_runs", "war",
            ],
        )

    logger.info("Batting WAR: {} players in {}", len(rows), season)
    return len(rows)


# Rough replacement-level RA/9 (a freely available reliever / AAA call-up).
REPLACEMENT_RA9 = 5.5
...
def _compute_pitcher_ra9(
    ...
    lg_outs, lg_runs = client.query(
        f"SELECT {outs_expr}, sum(post_bat_score - bat_score) FROM pitches"
        + _RUNS_FILTER,
    ...
    result = client.query(
        f"""
        SELECT pitcher, {outs_expr} AS outs_recorded,
...
def _compute_pitcher_re24(
    ...
    if not re_matrix:
        logger.warning("No RE matrix for {} — run 'compute re-matrix' first", season)
        return {}
    ...
    df = df.with_columns([
        pl.col("base_out_state").shift(-1).over(HALF_INNING_COLS).alias("next_state"),
    ...
    df = df.with_columns(
        (pl.col("re_post") - pl.col("re_pre")
         + pl.col("post_bat_score") - pl.col("bat_score"))
        .alias("run_value")
    )

    # Flip the sign: pitcher is credited when the batter's run value is low.
    pitcher_df = (
        df.group_by("pitcher")
        .agg([
        ...
        .filter(pl.col("bf") >= min_outs)
    )
    return {r["pitcher"]: -r["re24_raw"] for r in pitcher_df.iter_rows(named=True)}
...
def compute_pitching_war(
    ...
    """Pitching WAR via two methods (RA9-based and RE24-based); both are stored."""
    client.command(PITCHING_WAR_DDL)

    lg_ra9, ra9_rows = _compute_pitcher_ra9(client, season, min_ip)
    if not ra9_rows:
        logger.warning("No qualifying pitchers for {}", season)
        return 0

    re24_data = _compute_pitcher_re24(client, season, int(min_ip * 3))
    ...
    for pitcher, ip, ra9, ra9_war in ra9_rows:
        ...
        rows.append([
            pitcher, season,
            round(ip, 1), round(ra9, 2), round(lg_ra9, 2), round(ra9_war, 1),
            round(re24, 1), round(re24 / RUNS_PER_WIN, 1),
        ])
    ...
    client.insert(
        ...
        column_names=[
            "player_id", "season",
            "ip", "ra9", "lg_ra9", "ra9_war",
            "re24", "re24_war",
        ],
    )
    logger.info("Pitching WAR: {} pitchers in {}", len(rows), season)
    return len(rows)
